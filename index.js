...
//sanitize arra
function convertInput(funcInput){
...
const ingredient = convertInput(input);

//Get number of client and pop out from the input
const numberOfClient = Number(ingredient[0]);
ingredient.shift();
...
let likesDislike = -1;
console.log("Number of Clients", numberOfClient)

while(i < numberOfClient) {
    ...
    addIngredient(numberOfLikesIngredient, likesIngredient, likes);

    let numberOfDislikeIngredient = +dislikeIngredient[0];
    addIngredient(numberOfDislikeIngredient, dislikeIngredient, dislike);
...
function addIngredient(number, totalIngredient, likeOrDislike) {
    if(number > 0) {
        let k = 1;
...
//filter out the likes and dislikes
function createNewIngredient(_likes, _dislike) {
    for(let each of _dislike) {
        if(_likes.includes(each)){
            _likes.splice(_likes.indexOf(each), 1)
        }
    }
    
    return `${_likes.length} ${_likes.join(' ')}`;
}

console.log(createNewIngredient(likes, dislike))
